...
Dorminfo.updateroom = (room, result) => {
    // console.log("id dorm:",dorm.dorm_ID)
    for (let i = 0; i < room.length; i++) {
        db.query("UPDATE room SET room_Type = ?, room_Price = ? WHERE room_ID = ? ",
            [room[i].room_Type,room[i].room_Price,room[i].room_ID], (err, res) => {
                if (err) {
                    console.log("error: ", err);
                    result(err, null);
                    return;
                }
                console.log("Update dorm success!!!,response at item ",i)
                result(null, res);
            })
    }
